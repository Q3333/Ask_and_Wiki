(function($){
    var TIMEOUT = 8;  // movement timeout in seconds
    var CENTRE_FORCE = 0;  // strength of attraction to the centre by the active node
    ...
    var Node = function(obj, name, parent, opts) {
        ...
        this.el = $('<a href="'+this.href+'" data-content="'+this.data+'">'+this.name+'</a>');
...
//                    this.obj.ctx.lineWidth = "1";
        ...
        // var c = this.obj.canvas.path("M"+this.start.x+100 +' '+this.start.y+100 +"L"+this.end.x+100 +' '+this.end.y+100 ).attr({stroke: this.strokeStyle, opacity:0.2, 'stroke-width':'5px'});                
    }
...
$(document).ready(function() {
    ...
    var content = [];
    content['chest-pain'] = '<ul><li>chest pain sucks</li></ul>';
    content['head-pain'] = '<ul><li>head pain hurts</li></ul>';
    

    // add the data to the mindmap
    var root = $('body>div>ul>li').get(0).mynode = $('body').addRootNode($('body>div>ul>li>a').text(), {
        href:'/',
        url:'/',
...
//                    href:$('a:eq(0)',this).text().toLowerCase(),
            ...
            data:$('a:eq(0)',this).attr('data-content'),
            onclick:function(node) {
    ...
    $('body>div>ul>li>ul').each(function() { 
        $('>li', this).each(addLI);
    });
    ...
    $('a').click(function(e){
        ...
        //$('.alert .content').html($('.content').html()).parent().show();
        $('.alert .content-container').html($(tag).html()).parent().show();
        }
    });
